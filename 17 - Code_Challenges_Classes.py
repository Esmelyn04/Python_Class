"""
    ...
    Create a python class called DriveBot. Within this class, 
    create instance variables for motor_speed, sensor_range, and direction. 
    All of these should be initialized to 0 by default. After setting up the class, 
    create an object from the class called robot_1. Set the motor_speed to 5, the direction to 90, 
...
"""
    2. Adding Robot Logic
       Now we want to add some logic to our robot.

    - In the DriveBot class, add a method called control_bot which accepts parameters: 
      new_speed and new_direction. These should replace the associated instance variables.

    - Create another method called adjust_sensor which accepts a parameter called new_sensor_range 
      which replaces the sensor_range instance variable. Afterwards, use these methods to rotate 
      the robot 180 degrees at 10 miles per hour with a sensor range of 20 feet.
...
"""
    ...
    - Upgrade the constructor in the DriveBot class in order to accept three optional parameters. 
        - The constructor can accept motor_speed (which defaults to 0 if not provided), 
        - direction (which defaults to 180 if not provided, and 
        - sensor_range (which defaults to 10 if not provided). 
        - These parameters should replace the associated instance variables. 
        - Test out the upgraded constructor by initializing a 
            - new robot called robot_2 with a speed of 35, 
            - a direction of 75, 
...
""" Here are some examples of different ways to use the constructor: """
...
"""
    ...
    Set both of those variables to equal -999999. A third robot has been created below the first two robots.
    ...
    Hint:
    You can create class variables directly in the class without placing them in the constructor. 
    Also, to change the value of the class variable within all objects of the class, access the class variable directly from the class. 
    Here is an example: DriveBot.class_variable = 5.
...
"""
  ...
  For example, if three robots were created: first_robot, next_robot, and last_robot; 
  first_robot will have an id of 1 next_robot will have an id of 2 and last_robot will have an id of 3.

  Hint:
  Remember to make id an instance variable and to use another class variable to count the number of robots. 
  For example: a class variable called robot_count will be incremented every time the constructor is called 
  from the DriveBot class and the value will be assigned to id.
"""
